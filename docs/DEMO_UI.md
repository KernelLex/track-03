...
I published this at its own URL (title "TrueCommit Console", favicon 🎛️).
...
page, over the internet, to my own locally-running backend
(`agent/api/demo.py`'s `/demo/trigger`, reached through my `cloudflared`
...
1. `trucommit serve` + the `cloudflared` tunnel both need to be running
   ...
   a quick tunnel and *will* change if `cloudflared` restarts; I gave the
   page's settings panel an editable backend-URL field for exactly this
...
3. Open the dashboard, expand "Demo connection settings," paste in the
   backend URL and my `DEMO_TRIGGER_SECRET` (from `.env`). It's saved only
   in that browser's `localStorage` — never part of the page's own
   published source.

## The security tradeoff, stated plainly

`/demo/trigger` is reachable by anyone who has both the tunnel URL and the
secret — and the secret, once I paste it into the page, lives in my
...
on my own server, never a caller-supplied recipient, and I rate-limited it
to one trigger per channel per 20 seconds. Worst case if the link and
secret both leaked: someone could repeatedly message or call *me* while my
tunnel is up, not anyone else. I turn it off by stopping `trucommit serve`
(or unsetting `DEMO_TRIGGER_SECRET`) once I'm done demoing.
...
demand). I reply on my actual phone, and the dashboard: appends my real
