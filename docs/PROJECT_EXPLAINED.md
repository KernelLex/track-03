# TrueCommit, explained in full

The long version. [`README.md`](../README.md) is the short one — start there
if you have five minutes; this is what to read if you have thirty.

Razorpay AI Buildathon 2026, Track 03 (AI Revenue Recovery). Full spec:
[`DEVDOC_v6.md`](../DEVDOC_v6.md).
...
Every status in [`docs/evidence/REAL_BATCH.md`](evidence/REAL_BATCH.md)
...
comparison in [`docs/RESULTS.md`](RESULTS.md) is a **simulation
...
[`docs/evidence/REAL_SCENARIOS.md`](evidence/REAL_SCENARIOS.md).
...
1. **A pre-registration, locked before the run.**
   ...
   `eval/report.py` produced [`docs/RESULTS.md`](RESULTS.md) from
   ...
   → [`docs/RESULTS_FAMILY_B.md`](RESULTS_FAMILY_B.md)) exists because
...
3. **Every action goes through one chokepoint.** `check_bounds()` before
   ...
   ([`docs/evidence/DRY_RUN_BATCH.md`](evidence/DRY_RUN_BATCH.md)).

## Every number here, traced to a command and a test

[`docs/CLAIM_MATRIX.md`](CLAIM_MATRIX.md) maps every quantitative
...
1. **My extractor does not significantly beat a keyword regex on my own
   ...
   for. → [`EXTRACTION_ACCURACY.md`](evidence/EXTRACTION_ACCURACY.md)
...
9. **`BoundsContext.now` is a fixed date.** The demo layer never overrides
   ...
   actually made at 20:00 IST. → [`LIMITATIONS.md`](LIMITATIONS.md)
...
caught it → [`docs/WHAT_BROKE.md`](WHAT_BROKE.md). Four worth naming
...
[`docs/evidence/AT_RISK_HEADLINE.md`](evidence/AT_RISK_HEADLINE.md).
...
**A real, live pipeline runs unattended.** A `payment.failed` webhook triggers DIAGNOSE → DECIDE → BOUNDS → ACT automatically — no manual call, no dashboard click — live-verified against the actual running server: a real Razorpay payment link created, a real Telegram message sent with it. `dry_run=True` proves the identical real pipeline's judgment against a batch of invoices with zero rupees able to move — see [`docs/ORCHESTRATION.md`](ORCHESTRATION.md) and [`docs/evidence/DRY_RUN_BATCH.md`](evidence/DRY_RUN_BATCH.md) (500 decisions, spanning all 29 real diagnosis classes, 0 rail calls).
...
uv run pytest             # 1,477 collected: 1,466 run without credentials, 11 skipped (they run live with Razorpay test keys set)
```
...
([`docs/evidence/REAL_BATCH.md`](evidence/REAL_BATCH.md), ₹215,867 across
...
See [`docs/SETUP.md`](SETUP.md) for the verified clean-clone timing
and [`docs/LIMITATIONS.md`](LIMITATIONS.md) for the complete,
...
[`docs/WHATSAPP.md`](WHATSAPP.md)) remains a **separate, still-unbuilt
...
and every other Tier-1 document: see [`docs/`]().
