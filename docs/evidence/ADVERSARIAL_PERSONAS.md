...
Each of 100 synthetic personas run through the real `check_bounds()` gate under three mechanically-distinct exploit strategies, over a 90-day window. The one number that matters: **cases permanently stalled**, which must be 0 for the §24.2 fixes to be doing their job.
