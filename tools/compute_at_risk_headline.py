#!/usr/bin/env python3
"""My persona-free headline, DEVDOC_v6 §26: "Rs at risk on debits
...
**What I'm honest about here that §26's original framing (written
for a live book of business) doesn't have to be**: my build has no real
production mandate corpus yet, so the batch below is a synthetic
*construction* I built — I deliberately build a declared number of mandates
with a genuine, real object-shape defect (not drawn from any behavioural
model), and deliberately build the rest healthy. The **defect rate is
a demonstration parameter I chose, not a claim about real-world
prevalence** — I state that plainly in the output, not leave it implicit. What *is*
...
persona-behaviour content at all, unlike almost every other number my
project reports.

    uv run python tools/compute_at_risk_headline.py --n 1000

I write docs/evidence/AT_RISK_HEADLINE.md and a matching JSON snapshot.
"""
...
"""A construction parameter I declared, not a measured real-world rate --
see module docstring."""
EXPIRY_BREACH_RATE = 0.08
"""Same. Independent of HEADROOM_BREACH_RATE -- I can construct a mandate
with either, both, or neither defect."""
...
def render_markdown(results: list[dict]) -> str:
    ...
    lines = [
        ...
        "",
        f"I generate this with `tools/compute_at_risk_headline.py` at {now}.",
        "",
        f"**I constructed {len(would_fail)} of {n} synthetic mandates with a real, "
        ...
        f"guaranteed to fail before I ever checked this batch.**",
        "",
        ...
        "",
        f"My build has no real production mandate corpus yet, so I constructed this batch of {n} "
        f"**synthetically**: I deliberately built {HEADROOM_BREACH_RATE:.0%} with "
        f"a headroom breach, {EXPIRY_BREACH_RATE:.0%} with an expiry breach (independently, so "
        f"some carry both) — **construction parameters I declared, not a measured real-world "
        ...
        f"(`agent/mandate/health.py::check_mandate_health()`) that runs in my live orchestrator "
