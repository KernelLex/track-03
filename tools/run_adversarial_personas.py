#!/usr/bin/env python3
"""DEVDOC_v6 §24.3: run the three mechanically-distinct adversarial
personas (SERIAL_PROMISER, DISPUTE_ABUSER, CHANNEL_HOPPER — see
eval/personas/adversarial/strategies.py's own docstring for why INJECTOR
isn't simulated here) against the real check_bounds() gate, over a
population of synthetic personas, and report the one number DEVDOC_v6
actually asks for: cases permanently stalled (must be 0).

    uv run python tools/run_adversarial_personas.py --n 100

Writes docs/evidence/ADVERSARIAL_PERSONAS.md and a matching JSON snapshot.
"""
...
def render_markdown(results: dict[str, list[AdversarialRunResult]], *, n: int) -> str:
    lines = [
        ...
        "",
        f"Each of {n} synthetic personas run through the real `check_bounds()` gate under three "
        "mechanically-distinct exploit strategies, over a 90-day window. The one number that "
        "matters: **cases permanently stalled**, which must be 0 for the §24.2 fixes to be doing "
        "their job.",
        "",
    ...
    lines.append(
        ...
        "The fix: cooldown scales by `promise_credibility` "
    ...
    lines.append(
        ...
        "case reaches a human, it doesn't go silent. **A real finding surfaced while building this**: "
    ...
    lines.append(
        ...
        "commercial communication on any channel — an earlier version of this harness tagged it with "
        "the debtor's last-used, already-opted-out channel, which let the unrelated `TRAI_DND` rule "
        "block escalation too; fixed before this evidence was generated, not after)."
    )
    lines.append("")
    lines.append(
        "**`INJECTOR` is not simulated here.** Its exploit is prompt injection through free debtor "
        "text, which this harness has no live model call or free-text path to exercise at all — "
