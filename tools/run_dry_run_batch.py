#!/usr/bin/env python3
"""I run a real batch of decisions here, zero rupees moved: N synthetic invoices,
...
This is my concrete answer to "test mode success doesn't predict a real
...
I write docs/evidence/DRY_RUN_BATCH.md (human-readable) and
...
-- not a coarse 4-bucket stand-in. I sort these for a deterministic cycle
...
def render_markdown(results: list[dict], *, chain_ok: bool) -> str:
    ...
    lines = [
        ...
        "",
        f"I generate this with `tools/run_dry_run_batch.py` at {now}. {n} synthetic invoices, spanning all "
    ...
    lines.append("")
    lines.append(
        "Every row above came from a real `run_pipeline()` call — the same function my live "
        "webhook orchestrator (`agent/api/app.py`) and my real batch run "
        ...
        "(`agent/act/executor.py`) — I prove this, not just assert it: this script asserts zero rail "
        "objects were created before writing anything out."
    )
    lines.append("")
    if refused == 0:
        lines.append(
            "**Honest note on the 0% refusal rate**: I built this batch structurally incapable of "
            ...
            "either. In `tools/run_real_scenarios.py` (the *real*, non-dry-run batch) I deliberately "
            "include a scenario that **is** refused, against the real rail — see "
