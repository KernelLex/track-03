#!/usr/bin/env python3
"""A real batch run: 4 diverse scenarios pushed through the real pipeline
...
Writes docs/evidence/REAL_SCENARIOS.md (human-readable) and
...
This is priority #2 from the 2026-09-01 handoff document ("a real batch
run of 50+ synthetic invoices"), deliberately rescoped down to 4 curated,
maximally *distinct* scenarios per direct instruction on 2026-09-01 -- not
...
runs whose only variation is a random seed. Each scenario below is chosen
...
def _run_one(
    *, ledger: Ledger, outbound_store: OutboundActionStore, rail: RazorpayRail,
    ...
    """One real scenario: real p_base, real EV, a real BoundsContext built
    from this debtor's actual ledger history, and a real execute_action()
    call -- the exact same chokepoint every other action in this project
...
def render_markdown(results: list[dict]) -> str:
    ...
    lines = [
        ...
        "",
        f"Generated by `tools/run_real_scenarios.py` at {now}, against the real Razorpay "
    ...
    lines.append("")
    lines.append(
        "Four scenarios, not fifty: each one is chosen to exercise a *different* "
        ...
        "deliberately wrong action caught by check_bounds()) rather than many runs "
    ...
    if any_rail_error:
        lines.append("")
        lines.append(
            "**A real infrastructure limit was hit and handled, not hidden**: this "
            "Razorpay test account has a hard cap on test-mode payment links "
            "(`test mode limit of 30 reached`), reached during this session's earlier "
            "live-testing. Rather than crashing the batch, the failure is caught, "
            "recorded, and the run continues — the same discipline `agent/rails/"
            ...
            "type (a real, documented gap, not fixed in this pass)."
        )
    return "\n".join(lines)
